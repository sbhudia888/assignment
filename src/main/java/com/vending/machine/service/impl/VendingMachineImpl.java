...
public class VendingMachineImpl implements VendingMachine {
    ...
    public VendingMachineImpl(int initialQuantityToFill) {
        initialize(initialQuantityToFill);
    }

    //initialize machine with 5 coins of each denomination
    //and 5 cans of each Item
    private void initialize(int initialQuantityToFill) {

        for (Coin c : Coin.values()) {
            cashInventory.put(c, initialQuantityToFill);
        }

        for (Item i : Item.values()) {
            itemInventory.put(i, initialQuantityToFill);
        }
    ...
    public long selectItemAndGetPrice(Item item) {
        ...
        throw new ItemSoldOutException("Sold Out, Please buy another item");
    }
